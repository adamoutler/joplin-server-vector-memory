...
def test_temporal_weighting(temp_db, mock_ollama):
    ...
    cursor.execute(
        "UPDATE note_metadata SET updated_time = ? WHERE rowid = ?",
        (three_years_ago, rowid_a),
    )
    cursor.execute(
        "UPDATE note_metadata SET updated_time = ? WHERE rowid = ?", (now, rowid_b)
    )
    db.commit()

    # Search with target_date="3 years ago" and date_weight=1.0
    results_boosted_past = extract_result(
        search_notes("temporal topic", target_date="3 years ago", date_weight=1.0)
    )
    assert results_boosted_past[0]["title"] == "Note A"

    # Search with target_date="now" and date_weight=1.0
    results_boosted_now = extract_result(
        search_notes("temporal topic", target_date="now", date_weight=1.0)
    )
